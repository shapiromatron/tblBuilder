...
class EpiDescriptiveTables(DOCXReport):
    ...
    @classmethod
    def build_dual_field(cls, tbl, row, header, text='',
                         runs=None, colspan=None):
    ...
    def build_desc_tbl(self, d):
        ...
        # write title
        txt = u'Table X: Study description and methodologies: {}'\
            .format(d['reference']['name'])
        ...
        self.build_dual_field(tbl, row, 'Reference',
                              runs=runs, colspan=3)
        row += 1

        txt = d['studyDesign']
        self.build_dual_field(tbl, row, 'Study-design type',
                              text=txt, colspan=3)
        row += 1

        txt = u'{}; {}'.format(d['location'], d['enrollmentDates'])
        self.build_dual_field(tbl, row, 'Location and enrollment dates',
                              text=txt, colspan=3)
        row += 1

        txt = d.get('populationDescription', '')
        self.build_dual_field(tbl, row, 'Population description',
                              text=txt, colspan=3)
        row += 1
        ...
        if d['isCaseControl']:
            ...
            tbl.new_td_run(row, 2, [tbl.new_run('Population size',
                                                i=True, newline=False)])
            tbl.new_td_run(row, 3, [tbl.new_run('Source',
                                                i=True, newline=False)])
            row += 1
            ...
            self.build_dual_field(tbl, row, 'Eligibility criteria',
                                  text=txt, colspan=3)
            row += 1

            runs = [
                tbl.new_run('Population size: ', i=True, newline=False),
                tbl.new_run(u'{}'.format(d.get('populationSize', ''))),
                tbl.new_run('Loss-to-follow-up: ', i=True, newline=False),
                tbl.new_run(u'{}'.format(d.get('lossToFollowUp', ''))),
                tbl.new_run('Referent Group: ', i=True, newline=False),
                tbl.new_run(d.get('referentGroup', ''), newline=False),
            ]
            self.build_dual_field(tbl, row, 'Cohort details',
                                  runs=runs, colspan=3)
            row += 1

            txt = d.get('outcomeDataSource', '')
            self.build_dual_field(tbl, row, 'Outcome data source',
                                  text=txt, colspan=3)
            row += 1

        txt = d['exposureAssessmentType']
        self.build_dual_field(tbl, row, 'Exposure assessment',
                              text=txt, colspan=3)
        row += 1

        txt = d.get('exposureAssessmentNotes', '')
        self.build_dual_field(tbl, row, 'Exposure assessment notes',
                              text=txt, colspan=3)
        row += 1
        ...
        self.build_dual_field(tbl, row,
                              'Analysis methods and control for confounding',
                              runs=runs, colspan=3)
        row += 1
