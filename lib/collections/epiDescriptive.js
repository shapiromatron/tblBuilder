...
_.extend(EpiDescriptive, {
    ...
    wordContextByDescription: function(tbl_ids){
        ...
        descs.forEach(function(desc){
            desc.setWordFields();
            desc.results = _.chain(res)
                .filter(function(d){d.parent_id === desc._id})
                .each(function(d){d.setWordFields()})
                .value();
        });
...
tblBuilderCollections.attachSchema(EpiDescriptive, _.extend({
    ...
    exposureAssessmentNotes: {
        ...
        type: String,
        optional: true,
        popoverText: "Approach used to estimate exposure, if any (quantitative measurement, JEM questionnaire, expert judgement, biomonitoring, other)",
        textAreaRows: 4,
    },
    coexposures: {
