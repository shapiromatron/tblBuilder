...
_.extend(AnimalEvidence, {
    ...
    sortStudyDesign: function(objs, ascending){
        objs = _.chain(objs)
                .map(function(d){
                    d._studyDesignOrder = AnimalEvidence.studyDesigns.indexOf(d.studyDesign);
                    return d;
                })
                .sortBy("_studyDesignOrder")
                .value();
        if (!ascending) objs.reverse();
        return objs;
    },
    sortSpecies:    _.partial(libShared.sortByTextField, "species"),
    sortStrain:     _.partial(libShared.sortByTextField, "strain"),
    sortSex:        _.partial(libShared.sortByTextField, "sex"),
