...
shared = {
    getHTMLTitleBase: function() {
      ...
      return document.title = context + " Table Builder";
    },
    getHTMLTitleTbl: function() {
      var base, tbl;
      base = shared.getHTMLTitleBase();
      tbl = Session.get('Tbl');
      return document.title = base + " | " + tbl.tblType + " | " + tbl.name;
    },
    ...
    riskFormatter: function(obj) {
      ...
      if (isNumber(obj.riskLow) && isNumber(obj.riskHigh)) {
        txt += " (" + obj.riskLow + "–" + obj.riskHigh + ")";
      }
      if (obj.riskEstimated) {
        txt = "[" + txt + "]";
      }
      return txt;
    },
